...
class _ChangePasswordState extends State<ChangePassword> {
  ...
  changePassword() async {
    ...
    if((!oldPassController.text.contains(" ")) && oldPassController.text.length >= 6) {
      if((!newPassController.text.contains(" ")) && newPassController.text.length >= 6) {
        if((!confirmPassController.text.contains(" ")) && confirmPassController.text.length >= 6) {
          if(newPassController.text == confirmPassController.text) {
            ...
            // });
            //
            // final response = await api.changePassword(oldPassController.text, newPassController.text);
            //
            // setState(() {
            //   isApiCalling = false;
            // });
          ...
          helper.errorDialog(context, "Confirm password should be atleast 6 characters amd valid");
        }
      } else {
        helper.errorDialog(context, "New password should be atleast 6 characters and valid");
      }
    } else {
      helper.errorDialog(context, "Old password should be atleast 6 characters and valid");
    }
        ...
        child: SingleChildScrollView(
          child: Column(
            ...
            children: [
              ...
              customText.kText("Password Change", 30, FontWeight.bold, Colors.black, TextAlign.start),
              // SizedBox(
                ...
                child: Container(
                  ...
                  child: Center(
                    child: isApiCalling
                      ...
                      : customText.kText("Save & Change", 30, FontWeight.w700, Colors.white, TextAlign.center),
                  ),
                ),
                onTap: () {
                  changePassword();
